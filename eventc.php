...
function listeEvent () {
  ...
  try {
	  $idc=$_GET['id'];
    $sth = $dbh->prepare('SELECT * FROM `Event` WHERE `id`='. $idc .' ORDER BY `time` DESC, `TypeEvent`  DESC LIMIT 50');
...
<div id="eventcreature">
    <h2>Liste des Evènements de la Créature : <?php echo "$idc"; ?></h2>
	<?php

if (($_GET['id'])!=0) // On a une id
{	
